...
if [[ "$MODE" == "remote" ]]; then
  ...
  export PARTICLE_VIS_AUTO_FIT_ON_FIRST_LOAD="${PARTICLE_VIS_AUTO_FIT_ON_FIRST_LOAD:-1}"
  export PARTICLE_VIS_EGL_STRICT_CONTEXT="${PARTICLE_VIS_EGL_STRICT_CONTEXT:-1}"
  ...
  if [[ "${PARTICLE_VIS_REMOTE_TRANSPORT:-legacy}" == "bounded" ]]; then
    export PARTICLE_VIS_REMOTE_STILL_ENDPOINT="${PARTICLE_VIS_REMOTE_STILL_ENDPOINT:-tcp://127.0.0.1:5562}"
    echo "Remote stills: $PARTICLE_VIS_REMOTE_STILL_ENDPOINT"
  fi
